...
def process_log_data(spark, input_data, output_data):
    ...
    dfActions = df.filter(df.page == 'NextSong') \
    ...
    dfActions = dfActions.withColumn('timestamp', get_timestamp(dfActions.ts))
    
    
    # create datetime column from original timestamp column
    get_datetime = udf(lambda x: str(datetime.fromtimestamp(int(x) / 1000)))
    dfActions = dfActions.withColumn('datetime', get_datetime(dfActions.ts)) 
    
    # extract columns to create time table
    time_table = dfActions.select('datetime') \
                           .withColumn('start_time', dfActions.datetime) \
    ...
    joined_df = dfActions.join(song_df, col('log_df.artist') == col(
...
def main():
    ...
    output_data = "s3a://datalake-op-bucket/"
